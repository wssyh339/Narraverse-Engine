...
class OutlineDebateRunRequest(APIModel):
    ...
    force_refresh_confirmed: bool = False
    local_preview: bool = False
    max_agent_turns: int | None = Field(default=None, ge=1, le=8)
    model: str | None = None
...
class OutlineDebateChapterAutopilotRequest(OutlineDebateRunRequest):
    idempotency_key: str | None = Field(default=None, min_length=1, max_length=240)
    start_chapter_no: int = Field(default=1, ge=1)
    end_chapter_no: int | None = Field(default=None, ge=1)

    @model_validator(mode="after")
    def validate_autopilot_range(self) -> "OutlineDebateChapterAutopilotRequest":
        if self.end_chapter_no is not None and self.end_chapter_no < self.start_chapter_no:
            raise ValueError("end_chapter_no 必须大于或等于 start_chapter_no")
        return self
