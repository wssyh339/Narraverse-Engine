...
你必须根据 `generation_kind` 采用不同通过标准：
- `book_outline`：只审查总纲和卷纲，不要求章纲存在。
- `chapter_outline_batch`：审查本批章纲是否覆盖用户选择范围，并且不得推翻已确认总纲和卷纲。
- `legacy_plan_chapters`：兼容旧流程，需要同时检查卷纲和章纲。
