...
function eventTitle(event: OutlineDebateStreamEvent) {
  ...
  if (event.type === "artifact") return `生成候选：${event.artifact.title}`;
...
function eventDescription(event: OutlineDebateStreamEvent) {
  ...
  if (event.type === "pause") return event.next_agent_name ? `下一位候选：${agentDisplayName(event.next_agent_name)}` : event.message;
  if (event.type === "interrupt") return event.message;
  if (event.type === "decision") return event.decision.decision;
  if (event.type === "artifact") return `${event.artifact.type} · 生成候选 → 确认阶段自动入正典`;
...
function phaseStatus(session: OutlineDebateSession | null, phase: OutlineDebatePhase) {
...
export function OutlineDebatePanel({
  ...
  const allCandidatesConfirmed = Boolean(
    session &&
      (["book", "volumes", "chapters"] as OutlineDebatePhase[]).every(
        (phase) => session.phase_runs?.[phase]?.candidate_status === "confirmed" && session.confirmed_candidates?.[phase],
      ),
  );
  ...
  const confirmActivePhase = async () => {
    if (!session || !activeRun) {
      message.warning("请先形成阶段结论，再确认候选");
      return;
    }
    try {
      ...
      const result = await studioApi.confirmOutlineDebatePhase(projectId, session.id, activePhase, {
        ...
        notes: `${confirmActionLabels[activePhase]}${itemLabel ? `（${itemLabel}）` : ""}：用户在议事面板确认候选。`,
      });
      setSession(result.session);
      if (activePhase === "book") {
        onFormalCommit?.();
      }
      message.success(activePhase === "book" ? "总纲候选已确认并写入总纲正文" : `${itemLabel}已确认并更新正典`);
    } catch (error) {
      message.error(error instanceof Error ? error.message : "确认候选失败");
    }
  };

  const commitConfirmedCandidates = async () => {
    if (!session || !allCandidatesConfirmed) {
      message.warning("请先确认总纲、卷纲和章纲候选");
      return;
    }
    try {
      const result = await studioApi.commitOutlineDebateCandidates(projectId, session.id, {
        overwrite_existing_chapters: true,
        notes: "用户在议事面板确认三阶段候选后写入正式大纲。",
      });
      setSession(result.session);
      onFormalCommit?.();
  ...
  return (
    <section className={`outline-debate-panel ${compact ? "is-compact" : ""}`}>
      <div className="outline-debate-header">
        ...
        <Space wrap>
          {phaseStatus(session, "book")}
          {phaseStatus(session, "volumes")}
          {phaseStatus(session, "chapters")}
        </Space>
      </div>
      <div className="outline-debate-controls">
        ...
        <Space wrap>
          ...
          <Button type="primary" icon={<Play size={15} />} loading={running} onClick={() => runActivePhase({ refreshPhase: true })}>
            流式讨论
          </Button>
          <Button icon={<RefreshCw size={15} />} disabled={running} onClick={() => runActivePhase({ refreshPhase: true })}>
            刷新本阶段
          </Button>
        ...
        <div className="outline-debate-composer-dock">
          <div className="outline-debate-composer">
            ...
            <Space className="outline-debate-command-row" wrap>
              <Button icon={<MessageSquare size={15} />} disabled={running} onClick={postUserMessage}>
                发表意见
              </Button>
